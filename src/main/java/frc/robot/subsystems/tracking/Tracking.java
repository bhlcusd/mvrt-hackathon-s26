...
public class Tracking extends SubsystemBase<TrackingStates> {
    ...
    // The camera we track with, plus a handle to Drive (we need the robot's pose
    // and to set overrides).
    private final Limelight limelight;
    private final Drive drive;
    // The field's tag positions, loaded once. Used to look up where a tag is on the
    // field.
    private final AprilTagFieldLayout tagLayout = AprilTagFieldLayout.loadField(AprilTagFields.kDefaultField);

    private boolean enabled = true;

    // targetTagId = the tag we're going for. locked = we've actually seen it, so
    // stop re-choosing.
    ...
    private Tracking() {
        super("Tracking");
        // Build the tracking camera: NT name "limelight-mvrt", mounted 0.2m fwd / 0.3m
        // up / tilted up
    ...
    // Lifecycle step 1: read the camera every loop, then decide which tag we're
    // going for.
    @Override
    ...
    // Decide which tag to track. Until we've actually seen a tag, keep picking the
    // closest one to the
    // robot (by field position). Once we see it, "lock" so the choice stops
    // changing mid-track. The
    ...
    // Closest tag to the robot by straight-line field distance (uses the field
    ...
    // Where to drive to before we can see the tag: a point STANDOFF_m in front of
    // the tag, facing it.
    // Drive uses this as the "drive to pose" goal until the camera picks the tag
    // up.
    ...
    // Lifecycle step 2: the state machine. getCommand() is the current state;
    // setCommand(...) changes
    // it. firstLoop() is true only on the first loop after entering a state (good
    // for one-time setup).
    @Override
    ...
    // Lifecycle step 3: publish everything to the log so you can see it in
    // AdvantageScope.
    @Override
    public void outputPeriodic() {
        ...
        // The chosen tag's field pose, as a 0- or 1-length array (drop on the 3D field
        // to see it).
        ...
        // Draw a line from the camera to every tag it currently sees (camera -> tag,
        // camera -> tag, ...).
    ...
    // ============================================================================================
    // Older getTrackingSpeeds(...) overloads below. These were the earlier
    // hand-tuned versions; the
    // current drive-to-pose + track flow uses getTargetSpeeds() / getApproachPose()
    // further down and
    ...
    public ChassisSpeeds getTrackingSpeeds(double targetAngle, double currAngle, double maxVel, double tZ_offset,
            double tX_offset) {
        double rotSpeed = 0;
        double sideSpeed = 0;
        double forwardSpeed = 0;
        ...
        // tz_error/tx_error are negated distances (0 - getTz), so a tag ahead gives a
        // negative
        // error; these multipliers flip that back so the robot drives TOWARD the tag,
        // not away.
    ...
    public ChassisSpeeds getTrackingSpeeds(double currAngle) {
        ...
        // } else {
        // if (currAngle >= targetStartAngleLeft) { // This means that its closer to the
        // left
        // System.out.println(currAngle);
        // rotSpeed = -(currAngle - targetStartAngleLeft) * ROT_KP;
        // } else if (currAngle <= targetStartAngleRight) { // this means that its
        // closer to the right
    ...
    // Is that chosen tag in the camera's view right now? Drive uses this to switch
    // between "drive to
    ...
    // Where the chosen tag is relative to the robot, computed from odometry + the
    // field layout.
    // Returns forward distance and lateral (right-positive) offset, matching the
    // camera's tx/tz.
    public TrackingLocation getTargetLocation() {
        ...
        // tag position relative to the robot: subtract the robot's position, then
        // un-rotate by heading.
    ...
    // The final tracking move once the tag is in view: turn to its angle AND drive
    // in to STANDOFF_m.
    public ChassisSpeeds getTargetSpeeds(double currAngle, double maxVel) {
        ...
        // Only drive if we're not basically there. SharkFin is the approach curve
        // (smooth in, no overshoot).
        if (Math.abs(tz_error) > 0.02 || Math.abs(tx_error) > 0.02) {
    ...
    // Just the turn-to-the-tag part (Drive uses this to steer while driving to the
    // approach pose, so it
    ...
    // Shared heading controller: P on the wrapped error, clamped, with a deadband.
    // Degrees in, rad/s out.
